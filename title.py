    ...
    if protocol_type == 'SHADOWSOCKS':
        for element in array_configuration:
            ...
            if shadowsocks_match is None:
                ...
                if shadowsocks_match is None:
                    ...
                    with open("./splitted/no-match", "a", encoding="utf-8") as no_match_file:
        ...
        for element in array_configuration:
            ...
            for ip_address in ips_list:
                ...
                '''
                # Retrieve config network type and security type
                type_value = dict_params.get('type', '')
                if isinstance(type_value, bool):
                    config_type = 'TCP' if type_value else 'TCP'
                else:
                    config_type = str(type_value).upper() if type_value not in [None, ''] else 'TCP'
                security_value = dict_params.get('security', '')
                if isinstance(security_value, bool):
                    config_secrt = 'TLS' if security_value else 'NA'
                else:
                    config_secrt = str(security_value).upper() if security_value not in [None, ''] else 'NA'
        ...
        for element in array_configuration:
            ...
            for ip_address in ips_list:
                ...
                # Retrieve config network type and security type
                net_value = dict_params.get('net', '')
                if isinstance(net_value, bool):
                    config_type = 'TCP' if net_value else 'TCP'
                else:
                    config_type = str(net_value).upper() if net_value not in [None, ''] else 'TCP'
                tls_value = dict_params.get('tls', '')
                if isinstance(tls_value, bool):
                    config_secrt = 'TLS' if tls_value else 'NA'
                else:
...
def check_modify_config_parallel(array_configuration, protocol_type, check_connection=True, max_workers=30):
    if max_workers <= 1 or len(array_configuration) <= 1:
        return check_modify_config(
            array_configuration=array_configuration,
            protocol_type=protocol_type,
            check_connection=check_connection,
        )

    chunk_size = max(1, (len(array_configuration) + max_workers - 1) // max_workers)
    chunks = [
        array_configuration[index:index + chunk_size]
        for index in range(0, len(array_configuration), chunk_size)
    ]
    ...
    for result in results:
        modified_array.extend(result[0])
        tls_array.extend(result[1])
        non_tls_array.extend(result[2])
        tcp_array.extend(result[3])
        ws_array.extend(result[4])
        http_array.extend(result[5])
        grpc_array.extend(result[6])
...
def create_country_table(country_path):
    ...
    country_url_pattern = '[Subscription Link](https://raw.githubusercontent.com/hadifarajvand/telegram-config-vray/main/countries/{country_code}/mixed)'
    manual_country_names = {
        'XK': 'Kosovo',
    }

    country_code_name_url = list()
    for element in country_code_list:
        country_code = element.upper()
        country_url = country_url_pattern.format(country_code=element)
        ...
        if country_code == 'NA':
            ...
            try:
                ...
                country_name = manual_country_names.get(country_code, 'Unknown')

        country_code_name_url.append((country_code, country_name, country_url))
